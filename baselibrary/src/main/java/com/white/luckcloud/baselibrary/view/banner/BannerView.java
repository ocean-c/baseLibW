...
public class BannerView extends RelativeLayout {
    ...
    private void initAttribute(AttributeSet attrs) {
        ...
        if (mDotFocusDrawable == null) {
            mDotFocusDrawable = ContextCompat.getDrawable(mContext, R.drawable.shape_indicator_focus);
        }
        // 获取指示器的默认Drawable
        mDotNormalDrawable = array.getDrawable(R.styleable.BannerView_dotIndicatorNormal);
        if (mDotNormalDrawable == null) {
            mDotNormalDrawable = ContextCompat.getDrawable(mContext, R.drawable.shape_indicator_normal);
        }
        // 获取指示器的间距
