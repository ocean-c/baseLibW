package com.luck.luckcloud.baselibrary.http;
...
import com.luck.luckcloud.baselibrary.http.callback.HttpActivityLifecycleCallback;
import com.luck.luckcloud.baselibrary.http.config.BaseHttpConfig;
import com.luck.luckcloud.baselibrary.http.engine.IHttpEngine;
import com.luck.luckcloud.baselibrary.http.engine.OkHttpEngine;
import com.luck.luckcloud.baselibrary.utils.NetWorkHelper;
...
//    }
    ...
    public void execute(EngineCallback callBack) {
        ...
        if (mCheckNetState) {
            if (!NetWorkHelper.checkNetState(mContext)) {
                ...
                if (StringUtils.isEmpty(mNoNetworkTips)) {
                    callBack.onError(mContext, BaseHttpConfig.CODE_NETWORK_CONNECTIONLESS, "");
                } else {
        ...
        //多加一个中英文参数
        mParams.put("langCode", LANGUAGE);  //zh 中文 ，en 英语
        // 调用ponPreExecute方法去拼装公有参数
        callBack.onPreExecute(mContext, mParams, mHeader);
        // 是否Https方式
        if (mIsHttps) {
...
//        Call call = null;
        ...
        if (mType == TYPE_POST_STRING) {
            if (StringUtils.isEmpty(mContent)) {
                mContent = mGson.toJson(mParams);
            }
            mHttpEngine.postString(mContext, mUrl, mContent, mHeader, callBack);
        }
        // 下载文件
