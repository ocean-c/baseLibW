package com.white.luckcloud.baselibrary.http.interceptor;
...
//        return response.newBuilder().body(okhttp3.ResponseBody.create(mediaType, content)).build();
     ...
     */
    @NonNull
    private synchronized Response interceptMethod2(Chain chain) throws IOException {
        Request request = chain.request();
        long startTime = System.currentTimeMillis();
        Response response = chain.proceed(chain.request());
        long endTime = System.currentTimeMillis();
     ...
     * 打印请求信息
     */
    private synchronized void logRequest(Request request) {
        // 打印Header
        Headers headers = request.headers();
        if (headers != null && headers.size() > 0) {
            StringBuilder sb = new StringBuilder();
     ...
     * 打印返回的信息
     */
    private synchronized Response logResponse(Response response) {
        try {
